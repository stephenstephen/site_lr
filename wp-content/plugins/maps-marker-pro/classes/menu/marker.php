...
class Marker extends Menu {
	...
	protected function show() {
		...
		if ($id !== 'new') {
			...
			$settings['scheduleFrom'] = ($marker->schedule_from) ? get_date_from_gmt($marker->schedule_from) : null;
			$settings['scheduleUntil'] = ($marker->schedule_until) ? get_date_from_gmt($marker->schedule_until) : null;
										...
										<div class="mmp-marker-setting-input">
											...
											<select id="geocodingProvider">
												<optgroup label="<?= esc_attr__('Available providers', 'mmp') ?>">
													<option value="algolia" <?= MMP::$settings['geocodingProvider'] !== 'algolia' ?: 'selected="selected"' ?>>Algolia Places</option>
													<option value="photon" <?= MMP::$settings['geocodingProvider'] !== 'photon' ?: 'selected="selected"' ?>>Photon@MapsMarker</option>
