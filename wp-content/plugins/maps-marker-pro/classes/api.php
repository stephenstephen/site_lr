	...
	public function redirect_endpoints($wp) {
		...
		switch ($wp->query_vars['mapsmarkerpro']) {
			...
			case 'export':
				...
				MMP::get_instance('MMP\Export')->request();
				die;
			case 'geo-sitemap':
				if (!MMP::$settings['apiSitemap']) {
					break;
				...
				die;
			case 'google-places':
				MMP::get_instance('MMP\Google_Places')->request();
				die;
			case 'download_gpx':
				MMP::get_instance('MMP\Download')->download_gpx();
				die;
			case 'download_temp':
				MMP::get_instance('MMP\Download')->download_temp();
				die;
			case 'download_debug':
				MMP::get_instance('MMP\Download')->download_debug();
				die;
		}
