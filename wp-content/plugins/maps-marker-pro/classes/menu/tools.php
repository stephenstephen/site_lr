...
class Tools extends Menu {
	...
	public function init() {
		...
		add_action('wp_ajax_mmp_health_check', array($this, 'health_check'));
	...
	public function batch_marker() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-batch-marker') === false) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('Security check failed', 'mmp')
			));
		...
		if (!isset($settings['batch_marker_mode']) || !in_array($settings['batch_marker_mode'], array('all', 'markers', 'maps'), true)) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('Invalid request', 'mmp')
			));
		}
		if ($settings['batch_marker_mode'] === 'markers' && !isset($settings['batch_marker_markers'])) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('No markers selected', 'mmp')
			));
		}
	...
	public function assign_markers() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-assign-markers') === false) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('Security check failed', 'mmp')
			));
		...
		if (!$markers) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('No markers found', 'mmp')
			));
		}
		$marker_ids = array();
		foreach ($markers as $marker) {
		...
		wp_send_json(array(
			'success' => true,
			'message' => esc_html__("Markers successfully assigned to map {$map->id}", 'mmp'),
			'maps' => $this->get_map_list()
		));
	}
	...
	public function delete_all_maps() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-delete-all-maps') === false) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('Security check failed', 'mmp')
			));
		...
		wp_send_json(array(
			'success' => true,
			'message' => esc_html__('Maps deleted successfully', 'mmp')
		));
	}
	...
	public function delete_all_markers() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-delete-all-markers') === false) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('Security check failed', 'mmp')
			));
		...
		wp_send_json(array(
			'success' => true,
			'message' => esc_html__('Markers deleted successfully', 'mmp')
		));
	}
	...
	public function import() {
		...
		if (!isset($_FILES['file'])) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('File missing', 'mmp')
			));
		}
		$markers = array();
		if ($file_type === 'geojson') {
			$json = $upload->parse_json($_FILES['file']['tmp_name']);
			if ($json === false) {
				wp_send_json(array(
					'success' => false,
					'message' => esc_html__('File could not be read', 'mmp')
				));
			}
			if ($json === null) {
				wp_send_json(array(
					'success' => false,
					'message' => esc_html__('File could not be parsed', 'mmp')
				));
			}
			if (!isset($json['features']) || !is_array($json['features'])) {
				wp_send_json(array(
					'success' => false,
					'message' => esc_html__('No geographical data found', 'mmp')
				));
			}
			foreach ($json['features'] as $feature) {
				$markers[] = $db->build_marker($feature, true);
			}
		} else {
			$csv = $upload->parse_csv($_FILES['file']['tmp_name']);
			if ($csv === false) {
				wp_send_json(array(
					'success' => false,
					'message' => esc_html__('File could not be read', 'mmp')
		...
		foreach ($markers as $marker) {
			...
			if (!$test_mode && $geocoding_flag) {
				$result = $mmp_geocoding->getLatLng($marker['address']);
				if (!$result['success']) {
					$details[] = array(
						'status' => 3,
						'message' => esc_html__('Geocoding error') . ' (' . $result['message'] . ')'
					);
					continue;
				}
		...
		if (!$test_mode) {
			if (count($adding)) {
				foreach ($adding as $key => $marker) {
					$adding[$key]['id'] = 0;
				}
				$markers_batches = ceil(count($adding) / 1000);
				for ($i = 1; $i <= $markers_batches; $i++) {
					$batch_start = ($i - 1) * 1000;
					$batch = array_slice($adding, $batch_start, 1000);
					$result = $db->add_markers($batch);
					if ($result) {
						$assigns = array();
						$id = $wpdb->insert_id;
						if ($assignments) {
							foreach ($batch as $marker) {
								if ($assign_mode === 'fixed' || ($assign_mode === 'missing' && !count($marker['maps']))) {
									$assigns[$id] = $assign_maps;
								} else if (count($marker['maps'])) {
									$assigns[$id] = $marker['maps'];
								}
								$id++;
							}
							$db->assign_assoc($assigns);
						}
					}
				}
			}
			if (count($updating)) {
				foreach ($updating as $marker) {
					$db->update_marker((object) $marker, $marker['id']);
					if ($assignments) {
						$db->unassign_all_maps($marker['id']);
						if ($assign_mode === 'fixed' || ($assign_mode === 'missing' && !count($marker['maps']))) {
							$db->assign_maps_marker($assign_maps, $marker['id']);
						} else if (count($marker['maps'])) {
							$db->assign_maps_marker($marker['maps'], $marker['id']);
						}
					}
				}
			}
	...
	public function health_check() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-health-check') === false) {
			wp_send_json(array(
				'success' => false,
				'message' => esc_html__('Security check failed', 'mmp')
			));
		...
		<ul>
			...
			<li><?= esc_html__('WordPress version', 'mmp') ?>: <?= $debug_info['wp_version'] ?></li>
			<li><?= esc_html__('PHP version', 'mmp') ?>: <?= $debug_info['php_version'] ?></li>
			...
			<div id="maps_markers_tab_content" class="mmp-tools-tab">
				...
				<div id="batch_marker_section" class="mmp-tools-section">
					...
					<div>
						<form id="markerSettings" method="POST">
							...
							<select id="batch_marker_markers" name="batch_marker_markers[]" multiple="multiple">
								<?php foreach ($markers as $marker): ?>
									<option value="<?= $marker->id ?>">[<?= $marker->id ?>] <?= esc_html($marker->name) ?></option>
								<?php endforeach; ?>
							</select><br />
							...
							<select id="batch_marker_maps" name="batch_marker_maps[]" multiple="multiple">
								<?php foreach ($maps as $map): ?>
									<option value="<?= $map->id ?>">[<?= $map->id ?>] <?= esc_html($map->name) ?> (<?= $map->markers ?> <?= esc_html__('markers', 'mmp') ?>)</option>
								<?php endforeach; ?>
							</select><br />
				...
				<div id="assign_markers_section" class="mmp-tools-section">
					...
					<div>
						...
						<label>
							<?= esc_html__('Target', 'mmp') ?>
							<select id="assign_markers_map" name="assign_markers_map">
								<?php if ($maps !== null): ?>
									<?php foreach ($maps as $map): ?>
										<option value="<?= $map->id ?>">[<?= $map->id ?>] <?= esc_html($map->name) ?> (<?= $map->markers ?> <?= esc_html__('markers', 'mmp') ?>)</option>
									<?php endforeach; ?>
				...
				<div id="import_markers" class="mmp-tools-section">
					...
					<div>
						...
						<form id="import_form" method="POST">
							...
							<select id="assign_maps" name="assign_maps[]" multiple="multiple">
								<?php foreach ($maps as $map): ?>
									<option value="<?= $map->id ?>">[<?= $map->id ?>] <?= esc_html($map->name) ?> (<?= $map->markers ?> <?= esc_html__('markers', 'mmp') ?>)</option>
								<?php endforeach; ?>
							</select><br />
			...
			<div id="health_tab_content" class="mmp-tools-tab">
				<div id="health_section" class="mmp-tools-section">
					...
					<div>
						...
						<div id="health_check_result"></div>
						<button type="button" id="start_health_check" class="button button-primary" data-nonce="<?= wp_create_nonce('mmp-tools-health-check') ?>"><?= esc_html__('Start', 'mmp') ?></button>
					</div>
				</div>
			</div>
