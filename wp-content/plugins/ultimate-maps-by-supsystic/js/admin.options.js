...
jQuery(document).ready(function(){
	...
	if(typeof(umsActiveTab) != 'undefined' && umsActiveTab != 'main_page' && jQuery('#toplevel_page_'+ umsMainSlug).hasClass('wp-has-current-submenu')) {
		...
		subMenus.removeClass('current').each(function(){
			if(jQuery(this).find('a[href$="&tab='+ umsActiveTab+ '"]').size()) {
	...
	if(jQuery('.umsInputsWithDescrForm').size()) {
	...
	if(jQuery('#umsPopupGoToTopBtn').size()) {
...
function umsResetCopyTextCodeFields(selector) {
	...
	if(area.find('.umsCopyTextCode').size()) {
	...
	if(area.find('.umsMapShortCodeShell').size()) {
...
function umsInitStickyItem() {
	jQuery(window).scroll(function(){
		...
		if(wasSticking) {
			if(jQuery('#umsPopupGoToTop').size())
...
function umsInitMainPromoPopup() {
	if(!UMS_DATA.isPro) {
		...
		jQuery('.umsProOpt').change(function(e){
			...
			var parent = null;
			if(jQuery(this).parents('#umsPopupMainOpts').size()) {
				parent = jQuery(this).parents('label:first');
			} else if(jQuery(this).parents('.umsPopupOptRow:first').size()) {
