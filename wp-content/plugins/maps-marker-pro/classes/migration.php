...
class Migration {
	...
	public function check_migration() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-check-migration') === false) {
			return;
		}
						...
						case 'osm_mapnik':
							switch ($settings['openstreetmap_variants']) {
								case 'osm-blackandwhite':
									$shortcode_atts['basemapDefault'] = 'osmBw';
									break;
								case 'osm-de':
	...
	public function data_migration() {
		...
		if (!isset($_POST['nonce']) || wp_verify_nonce($_POST['nonce'], 'mmp-tools-migration') === false) {
			wp_send_json(array(
				'success' => false,
		...
		wp_send_json(array(
			'success' => true,
			'message' => esc_html__('Data migration completed successfully', 'mmp'),
			'maps' => $tools->get_map_list()
		));
	}
	...
	private function convert_settings($settings) {
		...
		$keys = array(
			...
			'geocodingAlgoliaAppId'             => 'geocoding_algolia_appId',
			'geocodingAlgoliaApiKey'            => 'geocoding_algolia_apiKey',
			'geocodingAlgoliaAroundLatLng'      => 'geocoding_algolia_aroundLatLng',
			'geocodingAlgoliaAroundLatLngViaIp' => 'geocoding_algolia_aroundLatLngViaIP',
			'geocodingAlgoliaLanguage'          => 'geocoding_algolia_language',
			'geocodingAlgoliaCountries'         => 'geocoding_algolia_countries',
			'geocodingPhotonLanguage'           => 'geocoding_photon_language',
			'geocodingPhotonBiasLat'            => 'geocoding_photon_location_bias_lat',
			'geocodingPhotonBiasLon'            => 'geocoding_photon_location_bias_lon',
			'geocodingPhotonFilter'             => 'geocoding_photon_filter_results',
			...
			'directionsYoursType'               => 'directions_yours_type_of_transport',
			'directionsYoursLayer'              => 'directions_yours_layer',
			'directionsOrsRoute'                => 'directions_ors_routeWeigh',
			'directionsOrsType'                 => 'directions_ors_routeOpt',
			'affiliateId'                       => 'affiliate_id',
			'betaTesting'                       => 'misc_betatest',
			'whitelabelBackend'                 => 'misc_whitelabel_backend',
			'backlinks'                         => 'misc_backlinks',
			'iconCustomShadow'                  => 'defaults_marker_icon_shadow_url_status',
			'iconCustomShadowUrl'               => 'defaults_marker_icon_shadow_url',
			...
			'iconShadowSizeX'                   => 'defaults_marker_icon_shadowsize_x',
			'iconShadowSizeY'                   => 'defaults_marker_icon_shadowsize_y',
			'iconShadowAnchorX'                 => 'defaults_marker_icon_shadowanchor_x',
			'iconShadowAnchorY'                 => 'defaults_marker_icon_shadowanchor_y',
			...
			'adminNotices'                      => 'misc_global_admin_notices',
		...
		if (isset($settings['geocoding_provider'])) {
			switch ($settings['geocoding_provider']) {
				case 'algolia-places':
					$converted['geocodingProvider'] = 'algolia';
					break;
				case 'photon':
					$converted['geocodingProvider'] = 'photon';
					break;
		...
		if (isset($settings['directions_yours_route_type'])) {
			$converted['directionsYoursRoute'] = ($settings['directions_yours_route_type'] == '1') ? 'shortest' : 'fastest';
		}
	...
	private function convert_layer($layer, $settings) {
		...
		if ($settings['controlbox_osm_mapnik'] == '1') {
			switch ($settings['openstreetmap_variants']) {
				case 'osm-blackandwhite':
					$converted['settings']['basemaps'][] = 'osmBw';
					break;
				case 'osm-de':
		...
		switch ($layer->basemap) {
			case 'osm_mapnik':
				switch ($settings['openstreetmap_variants']) {
					case 'osm-blackandwhite':
						$converted['settings']['basemapDefault'] = 'osmBw';
						break;
					case 'osm-de':
