...
class DB {
	...
	public function create_tables() {
		...
		$wpdb->query(
			"CREATE TABLE IF NOT EXISTS `{$wpdb->prefix}mmp_maps` (
				...
				`geojson` mediumtext COLLATE utf8_unicode_ci NOT NULL,
	 ...
	 * @return object|null Map object or null if no result is found
	 */
	 ...
	 * @param bool $count (optional) Whether to count the assigned markers
	 * @return array List of map objects
	 */
	...
	public function add_maps($data) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}mmp_maps (" . implode(',', array_keys($sanity)) . ") VALUES ";
		foreach ($data as $map) {
			$sql .= '(' . implode(',', array_values($map)) . '),';
		}
	 ...
	 * @return object|null Marker object or null if no result is found
	 */
	 ...
	 * @return array List of marker objects
	 */
	...
	public function add_markers($data) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}mmp_markers (" . implode(',', array_keys($sanity)) . ") VALUES ";
		foreach ($data as $marker) {
			$sql .= '(' . implode(',', array_values($marker)) . '),';
		}
		$sql = substr($sql, 0, -1); // Remove trailing comma from loop-generated query
	...
	public function update_marker($data, $id) {
		...
		$update = $wpdb->update(
			...
			array('id' => $id),
			array('%s', '%s', '%f', '%f', '%f', '%s', '%s', '%s', '%d', '%s', '%s', '%s', '%s'),
			array('%d')
		);
	...
	public function assign_markers($map_id, $marker_ids) {
		...
		$sql = "INSERT INTO {$wpdb->prefix}mmp_relationships ({$cols}) VALUES ";
	 ...
	 * @return object|null Layer object or null if no result is found
	 */
	 ...
	 *
	 * @since 4.0
	 * @return array List of layer objects
	 */
	 ...
	 * @param boolean $geojson (optional) Whether the data is in GeoJSON format
	...
	private function parse_marker_filters($filters, $group = true) {
		...
		if (isset($filters['scheduled']) && $filters['scheduled'] === false) {
			$query .= ' AND (markers.schedule_from IS NULL OR markers.schedule_from <= NOW()) AND (markers.schedule_until IS NULL OR markers.schedule_until >= NOW())';

		}
		if ($group) {
			$query .= ' GROUP BY markers.id';
		}
		if (isset($filters['orderby']) && (in_array($filters['orderby'], array('created_by', 'updated_by')) || array_key_exists($filters['orderby'], $this->prepare_markers()))) {
			$query .= " ORDER BY {$filters['orderby']} ";
			$query .= (isset($filters['sortorder']) && $filters['sortorder'] === 'desc') ? 'DESC' : 'ASC';
		}
		if (isset($filters['limit'])) {
